...
def main(argv):
	...
	parsed.data_dir += "/" if not parsed.data_dir.endswith("/") else ""
	...
	BG = pd.read_csv(file_background, delimiter='\t', 
					usecols=[0,1,3,4,7,9,11], header=None, 
					names=['Chr','Hop_pos','Hop_ID','Reads','Orf_pos','Orf','Strand'])
	...
	for file_in in files_experiment:
		...
		X = pd.read_csv(parsed.data_dir + file_in_basename +'.orf_hops', 
					delimiter='\t', usecols=[0,1,3,4,7,9,11], header=None, 
					names=['Chr','Hop_pos','Hop_ID','Reads','Orf_pos','Orf','Strand'])
		Y = cluster_orf2hops(X, BG, T[file_in_basename], T[file_basename_background], parsed.dist_cutoff)
		Y.to_csv(parsed.data_dir + file_in_basename +'.orf_peaks.'+ str(parsed.dist_cutoff) +'bp', sep='\t', index=False)
