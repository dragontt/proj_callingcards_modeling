...
def make_gnashyfile(bcfilename,cc_filelist,outpath):
    ...
    reader = csv.reader(open(cc_filelist, 'r'),delimiter = '\t')
    filelist_dict = {}
    for row in reader:
      filelist_dict[row[1]] = row[0]

    #initialize quality control dictionary
    qc_dict = {}
    #LOOP THROUGH EXPERIMENTS
    #loop through experiments and make a separate gnashy file for each
    for expt in sorted(filelist_dict.keys()):
      ...
      basename = '../SRA_files/'+filelist_dict[expt]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c','--cc_filelist',default='../SRA_files/cc_filelist.txt')
    ...
    make_gnashyfile(args.barcodefile,args.cc_filelist,args.outputpath)
